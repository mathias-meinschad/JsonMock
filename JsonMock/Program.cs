...
Log.Logger = new LoggerConfiguration()
	...
	.WriteTo.File(
		path: Path.Combine(appBase, "logs", "mock-.log"),
...
try {
	...
	app.MapPost("/scaJsonMock/startAuthentication", async (HttpRequest req) =>
		await GetResponseByJsonPath(req, "metaData.businessMetaData.correlationId", $"{scaBasePath}\\startAuthentication"));
	...
	app.MapPost("/scaJsonMock/notifyAuthenticationUpdate", async (HttpRequest req) =>
		await GetResponseByJsonPath(req, "metaData.businessMetaData.correlationId", $"{scaBasePath}\\notifyAuthenticationUpdate"));
	...
	app.MapPost("/pdgJsonMock/authorizeTokenProvisioning", async (HttpRequest req) =>
		await GetResponseByJsonPath(req, "metaData.businessMetaData.correlationId", $"{pdgBasePath}\\authorizeTokenProvisioning"));

	app.MapPost("/pdgJsonMock/notifyTokenDigitization", async (HttpRequest req) =>
		await GetResponseByJsonPath(req, "metaData.businessMetaData.correlationId", $"{pdgBasePath}\\notifyTokenDigitization"));

	app.MapPost("/pdgJsonMock/notifyTokenEvent", async (HttpRequest req) =>
		await GetResponseByJsonPath(req, "metaData.businessMetaData.correlationId", $"{pdgBasePath}\\notifyTokenEvent"));

	app.MapPost("/pdgJsonMock/activationCodeNotif", async (HttpRequest req) =>
		await GetResponseByJsonPath(req, "metaData.businessMetaData.correlationId", $"{pdgBasePath}\\activationCodeNotif"));
...
async Task<IResult> GetResponseByJsonPath(HttpRequest req, string path, string basePath) {
	// Read JSON body
